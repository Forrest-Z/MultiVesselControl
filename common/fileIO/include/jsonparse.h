...
body-fixed coordinate (BODY-G), whose origin located at the center of
gravity
*/
...
class jsonparse {
 ...
 private:
  ...
  void parsecontrollerdata() {
    ...
    // std::cout << "debug 函数 thrust allocation 1 " << std::endl;
    // thrustallocationdata_input.Q_sway =
    //     file["thrustallocation"]["penality"]["sway"].get<double>();
    // std::cout << "debug 函数 thrust allocation 2 " << std::endl;
