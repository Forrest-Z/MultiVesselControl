...
int main() {
  ...
  Eigen::Vector3d target_position =
      (Eigen::Vector3d() << 100, 200, 1).finished();
  state_type x = (state_type() << 0, 0, 0.1, 0, 0, 0).finished();
  Eigen::Matrix3d P =
  ...
  for (int i = 0; i != total_step; ++i) {
    u = -P * (x.head(3) - target_position);
    // u(0) = x(0);
    // u(1) = x(1);
    x = _simulator.simulator_onestep(0, u).getX();
    save_x.col(i) = x;
    std::cout << x.head(3) - target_position << std::endl;
    std::cout << "********************" << std::endl;
  }
  //画图**********************************************************************
  Gnuplot gp;
  ...
  gp << "set multiplot layout 6, 1 title 'Simulation results' font ',14'\n";
